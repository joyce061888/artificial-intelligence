...
def completion_query(prompt,max_tokens=10,temperature=0.95,samples=1):
    ...
    words = "null"
    command = f'curl {url}'+" -d '{"+'"data": [ "Completions","'+prompt+'",'+f"{max_tokens},{temperature},{samples},{words}"+"]}' -H"+' "Content-Type: application/json"'
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python query_gptj.py mode input")
        return
    mode = sys.argv[1]
    if mode == "completions":
        ...
        prompt = sys.argv[2]
        if len(sys.argv) < 4:
            print("Usage: python query_gptj.py words prompt wordlist\nWords in word list should be separated by semi-colons.")
            return
        words = sys.argv[3]
        ...
        prompt = sys.argv[2]
        if len(sys.argv) < 4:
            print("Usage: python query_gptj.py tokens prompt top_n\ntop_n should be an integer")
            return
